...
const NewsNotif = () => {
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-8 right-0 left-0 sm:right-8 sm:left-auto z-50 w-full sm:max-w-[480px] mx-3 sm:mx-0 bg-zinc-900 border border-zinc-800 shadow-2xl rounded-xl animate-fade-in overflow-hidden">
      {/* Header */}
      <div className="flex justify-between items-center px-4 py-3 sm:px-6 sm:py-4 bg-gradient-to-r from-black to-zinc-800 text-white">
        <div className="flex-1">
          <h2 className="text-base sm:text-lg font-bold">Announcement</h2>
          <p className="text-xs sm:text-sm text-gray-300">Stay informed with real-time updates</p>
        </div>
      ...
      {/* Body */}
      <div className="px-4 py-3 sm:px-6 sm:py-4 text-gray-100 max-h-[50vh] sm:max-h-[60vh] overflow-y-auto text-sm sm:text-base">
        {editMode ? (
          <textarea
            value={news}
            ...
            disabled={loading}
            rows={5}
            className="w-full p-2 sm:p-3 text-sm sm:text-base border border-zinc-700 bg-zinc-800 text-white rounded-md resize-none focus:ring-2 focus:ring-pink-500 outline-none"
          />
        ) : (
